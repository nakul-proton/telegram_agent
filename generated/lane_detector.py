...
Usage:
    ...
    python scripts/lane_detector.py --export        # process video and export diagnostic frames
"""
...
OUTPUT_FRAMES_DIR = PROJECT_ROOT / "output" / "frames"
...
def process_video(src: Path, export_override: bool = False) -> Path:
    v = CONFIG["video"]
    ...
    export_enabled = export_cfg.get("enabled", False) or export_override
    export_interval = export_cfg.get("interval", 100)
    export_types = export_cfg.get("types", ["annotated", "mask", "roi"])

    if export_enabled:
        video_frames_dir = OUTPUT_FRAMES_DIR / src.stem
        video_frames_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"Frame export enabled. Saving to {video_frames_dir} every {export_interval} frames.")
    ...
    try:
        while True:
            ...
            annotated_frame = annotate(frame, lanes)
            writer.write(annotated_frame)

            if export_enabled and frame_idx % export_interval == 0:
                for t in export_types:
                    if t == "annotated":
                        cv2.imwrite(str(video_frames_dir / f"frame_{frame_idx:06d}_annotated.png"), annotated_frame)
                    elif t == "mask":
                        cv2.imwrite(str(video_frames_dir / f"frame_{frame_idx:06d}_mask.png"), mask)
                    elif t == "roi":
                        cv2.imwrite(str(video_frames_dir / f"frame_{frame_idx:06d}_roi.png"), roi)
...
def main(argv: list[str]) -> int:
    ...
    if "--export" in argv:
        ...
        argv = [arg for arg in argv if arg != "--export"]

    arg = argv[1] if len(argv) > 1 else None

    try:
        src = find_input_video(arg)
        process_video(src, export_override=export_override)
